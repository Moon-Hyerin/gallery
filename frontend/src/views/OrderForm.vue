...
//주문 데이터 제출
const submit = async () => {
  if(state.form.payment !== 'card'){ //결제 수단이 카드가 아니라면
  ...
  if(res.status === 200){
    ...
    if(state.form.payment === "bank"){
      const price = computedTotalPrice.value.toLacaleString();
      message.push(`한국은행 123-456789-777 계좌로 ${price}원을 입금 해주시기 바랍니다.`);
    }
      ...
      <div class="row g-5">
        <div class="col-md-5 col-lg-4 order-md-last">
          <div class="mb-3">
            ...
            <span class="badge pg-primary rounded-pill align-middle">
              {{state.items.length}}
            </span>
          </div>
